...
export function AgentProductCard({ invocation }: AgentProductCardProps) {
  ...
  const image = output.product.images[0];
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card">
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {formatPrice(output.product.price, output.product.currency)}
            </p>
          </div>
        </div>
        ...
        {output.product.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {output.product.tags.slice(0, 5).map((tag) => (
              <span
                key={tag}
        ...
        <Link
          href={`/products/${output.product.slug}`}
